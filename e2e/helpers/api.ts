...
/**
 * Get lobby summary for the authenticated user.
 */
export async function getLobbySummaryApi(
	jwt: string
): Promise<{ owned: { id: number }[]; joined: { id: number }[] }> {
	const res = await apiFetch(jwt, '/lobbies/summary');
	return res.json();
}
