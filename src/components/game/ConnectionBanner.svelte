<script lang="ts">
	 ...
	 * during automatic reconnection attempts, a manual "Reconnect" button
	 * once all automatic retries are exhausted, and a warning when incoming
	 * messages fail to parse (stale board state).
	 */
	interface Props {
	...
	let { reconnecting, retriesExhausted, parseError, onReconnect }: Props = $props();
</script>
...
{:else if parseError}
	<div
		class="fixed top-0 right-0 left-0 z-50 flex items-center justify-center gap-3 bg-orange-600/90 px-4 py-2 text-sm font-medium text-white backdrop-blur-sm"
		role="alert"
	>
		<span>Sync error — board may be stale.</span>
		<button
			onclick={onReconnect}
			class="cursor-pointer rounded bg-white/20 px-3 py-1 text-sm font-semibold transition-colors hover:bg-white/30"
		>
